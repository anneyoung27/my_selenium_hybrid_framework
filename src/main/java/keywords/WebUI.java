...
public class WebUI {
    ...
    @Step("Check data: {1} in Table by column {2}") // 1 means value position in index in parameter, 2 means columnName position in index in parameter
    ...
    @Step("Open URL: {0}") // 0 means url position in index in parameter
    ...
    @Step("Click element {0}") // 0 means by position in index in parameter
    ...
    public static boolean dragAndDropOffset(By fromElement, int X, int Y) {
        try {
            Actions action = new Actions(DriverFactory.getDriver());
            // From the position of the first mouse click (clickAndHold), move to the position (X,Y) offset
    ...
    public static WebElement highLightElement(By by) {
        // Colour the outer border of the specified element - red (can be changed to another colour)
        if (DriverFactory.getDriver() instanceof JavascriptExecutor) {
            ((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].style.border='2px solid red'", getWebElement(by));
            sleep(STEP_TIME);
        }
    ...
    public static WebElement highLightElement(By by, String colorName) {
        // Colour the outer border of the specified element - red (can be changed to another colour)
        if (DriverFactory.getDriver() instanceof JavascriptExecutor) {
            ((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].style.border='2px solid " + colorName + "'", getWebElement(by));
            sleep(STEP_TIME);
        }
        return getWebElement(by);
    }
    ...
    public static void waitForElementVisible(By by) {
        try {
            WebDriverWait wait = new WebDriverWait(DriverFactory.getDriver(), Duration.ofSeconds(ConfigData.EXPLICIT_WAIT_TIMEOUT), Duration.ofMillis(500));
    ...
    public static void waitForElementVisible(By by, int timeOut) {
        try {
            WebDriverWait wait = new WebDriverWait(DriverFactory.getDriver(), Duration.ofSeconds(EXPLICIT_WAIT_TIMEOUT), Duration.ofMillis(500));
            wait.until(ExpectedConditions.visibilityOfElementLocated(by));
        } catch (Throwable error) {
    ...
    public static Boolean checkElementExist(WebDriver driver, By by) {
        List<WebElement> listElement = DriverFactory.getDriver().findElements(by);
     ...
     * Please wait for the page to load (Javascript loading)
     */
    public static void waitForPageLoaded(WebDriver driver) {
        WebDriverWait wait = new WebDriverWait(DriverFactory.getDriver(), Duration.ofSeconds(PAGE_LOAD_TIMEOUT), Duration.ofMillis(500));
        JavascriptExecutor js = (JavascriptExecutor) DriverFactory.getDriver();
